---
description: "Fes un donatiu i ajuda a fer millors mapes junts!"
extra:
  donate:
    info:
      description: "Fes un donatiu per a sostenir el projecte"
      title: "Creat amb amor per voluntaris entusiàstics"
    liberapay:
      title: "Dona via Liberapay"
    open_collective:
      title: "Dona via Open Collective"
    title: Donatius
  funding:
    items:
      - media: /images/donate/infrastructure.webp
        text: "Mantenim servidors ràpids per a poder actualitzar les dades de mapes i que pugueu descarregar-les sense retards. Les transferències de dades de mapes són grans quantitats d'informació, i cada mes n'augmenten."
        title: Infraestructura
      - media: /images/donate/support.webp
        text: "El suport a l'usuari, correcció d'errors i la millora de l'estabilitat de l'aplicació són les nostres prioritats principals. La llista de sol·licituds i informes d'errors augmenten cada dia i hi ha moltes sol·licituds de suport de l'App Store, Google Play i els correus electrònics de suport."
        title: Suport
      - media: /images/donate/developers.webp
        text: "De moment, tot el desenvolupament és realitzat per voluntaris amb temps limitat. Per a implementar característiques més ambicioses i grans, pagar els desenvolupadors podria ser necessari en un futur."
        title: Desenvolupadors
    title: "Com s'usaran els fons?"
  hero_description: "Donate and create better maps together!"
  info:
    - icon: static/images/icons/donate/icon01.svg
      text: "Depenem de les vostres donacions per a mantenir CoMaps gratuït"
    - icon: static/images/icons/donate/icon02.svg
      text: "Som una comunitat contribuint en el nostre temps lliure. Estimem el que fem, i estimem els nostres usuaris"
    - icon: static/images/icons/donate/icon03.svg
      text: "Amb el teu suport, treballem cap a una navegació enfocada a la privacitat i que sigui l'opció preferida al mercat"
  service_text: ~
  service_title: "Created with love by enthusiast volunteers"
template: donate.html
title: Donatius
weight: 10
---
